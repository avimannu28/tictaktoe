  ...
  return ( 
   <Container className='p-5'>
    ...
    <Row>
      ...
      {winMessage ? (
        <div className='mb-2 mt-2'><h1 className='text-primary text-uppercase text-center'>{winMessage}</h1><Button color='success' block onClick={reloadGame}>Reload Game</Button></div>
      ) : (
        ...
        {itemArray.map((item,index)=>(
          <Card onClick={()=>changeItem(index)} color='warning'>
            <CardBody className='box'>
              <Icon name={item} className='icon'></Icon>
            </CardBody>
          </Card>
        ))}
      </div>
